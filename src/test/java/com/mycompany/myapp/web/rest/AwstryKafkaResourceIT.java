...
class AwstryKafkaResourceIT {
    ...
    void setup() {
        ...
        AwstryKafkaResource kafkaResource = new AwstryKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/awstry-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/awstry-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
